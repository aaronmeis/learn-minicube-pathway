...
def process_task(body: bytes) -> str:
    ...
    Expected JSON format:
        ...
        {"type": "text", "operation": "reverse", "value": "hello"}
        {"type": "text", "operation": "upper", "value": "hello"}
    """
    try:
        task = json.loads(body)
    except json.JSONDecodeError:
        return f"ERROR: Invalid JSON: {body.decode()}"
    ...
    if task_type == "math":
        ...
        return f"ERROR: Unknown task type '{task_type}'. Use 'math' or 'text'."


def _process_math(task: dict) -> str:
    expr = task.get("expr", "")
    if not expr:
        return "ERROR: 'expr' field is required for math tasks."
    try:
        result = sympy.sympify(expr)
        return f"MATH RESULT: {expr} = {result}"
...
def _process_text(task: dict) -> str:
    ...
    if not value:
        return "ERROR: 'value' field is required for text tasks."

    if operation == "reverse":
        return f"TEXT RESULT: reverse('{value}') = '{value[::-1]}'"
    elif operation == "upper":
        return f"TEXT RESULT: upper('{value}') = '{value.upper()}'"
    elif operation == "lower":
        return f"TEXT RESULT: lower('{value}') = '{value.lower()}'"
    elif operation == "length":
        return f"TEXT RESULT: length('{value}') = {len(value)}"
    else:
        return f"ERROR: Unknown text operation '{operation}'. Use reverse/upper/lower/length."
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                ...
                sys.exit(1)
            print(f"[-] Connection failed. Retrying in {delay}s...", flush=True)
            time.sleep(delay)
...
def main():
    ...
    print(f"[*] Waiting for tasks on queue '{QUEUE_NAME}'. Press CTRL+C to exit.", flush=True)
